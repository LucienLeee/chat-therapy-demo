...
def load_model():
    model = (
        AutoModelForCausalLM.from_pretrained("./chat-therapy", trust_remote_code=True)
        .to(torch.bfloat16)
        .cuda()
    )
    tokenizer = AutoTokenizer.from_pretrained("./chat-therapy", trust_remote_code=True)
...
def combine_history(prompt):
    ...
    meta_instruction = (
        "你是一位拥有二十年从业经验的心理咨询师。你旨在通过专业心理咨询，帮助来访者解决心理问题。确保咨询连贯、专业、与来访者共情，并致力于达成最佳的治疗效果。"
    )
...
def main():
    ...
    st.title("话疗（chat-therapy）- demo AI心理咨询")
